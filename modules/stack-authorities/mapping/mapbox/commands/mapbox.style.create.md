...
**Style development can be parallelized:**

```
Phase 1: Preparation (PARALLEL)
┌──────────────────────────────────────────────┐
│ Task A: Define Style Metadata                │
│         - Set name, version, type            │
│                                              │
│ Task B: Gather Data Sources                  │
│         - List required tilesets             │
│         - Verify source URLs                 │
│                                              │
│ Task C: Plan Layer Hierarchy                 │
│         - Define layer order                 │
│         - Identify layer types needed        │
└──────────────────┬───────────────────────────┘
                   ↓
Phase 2: Layer Creation (SEQUENTIAL - order matters)
• Background layer
• Fill layers (polygons)
• Line layers (roads, boundaries)
• Symbol layers (labels, icons)
                   ↓
Phase 3: Styling & Testing (PARALLEL)
┌──────────────────────────────────────────────┐
│ Task A: Data-Driven Styling                  │
│         - Add expressions                    │
│         - Configure zoom stops               │
│                                              │
│ Task B: 3D Features                          │
│         - Add terrain/buildings              │
│         - Configure lighting                 │
└──────────────────┬───────────────────────────┘
                   ↓
Phase 4: Validation & Publish (SEQUENTIAL)
• Preview at multiple zooms/locations
• **Review quality** - Invoke `mapbox.style-reviewer` agent (optional):
  ```
  "Review style for Mapbox best practices compliance and optimization opportunities"
  ```
• Publish style
```
