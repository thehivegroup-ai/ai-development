...
- User needs to validate styles before production
- User asks about accessibility (WCAG, colorblind-safe)
- User needs to optimize style performance
- User wants to compare style versions
- User debugging style issues

**Not for:**

- Creating new styles or style recipes (use mapbox-style-patterns)
- Data visualization patterns (use mapbox-data-visualization-patterns)
- Framework performance optimization (use mapbox-web-performance-patterns)
- Studio workflows (use mapbox-standards)

**See also:**

- mapbox-style-patterns - Style recipes and layer configurations
- mapbox-web-performance-patterns - Framework-level optimization
- mapbox-standards - Studio workflows

---

## Token Optimization: Use Subagent for Comprehensive Reviews

**For complete style quality review, offload to subagent:**

```
Launch mapbox.style-reviewer agent:
"Review Mapbox style [style-id or JSON] for quality and performance.
Analyze: source limits, layer organization, expression validation,
text contrast (WCAG), performance optimization opportunities.
Generate quality report with prioritized findings."
```

**When to use subagent:**

- ✅ Comprehensive pre-production review
- ✅ Multiple validation checks (expressions + GeoJSON + contrast + optimization)
- ✅ Full style audit for quality gate

**When to keep in main conversation:**

- ❌ Single validation (just check one expression)
- ❌ Iterative debugging with user feedback
- ❌ Quick spot check

**Token savings:** 15-25K tokens (validation runs in isolated context)

**Time savings:** 30-40% via parallel MCP tool execution (agent can run multiple validations concurrently)
