...
- Choosing which search tool to use (use mapbox-search-patterns)
- Need parameter guidance for search tools (use mapbox-search-patterns)
- MCP bulk geocoding operations (use mapbox-mcp-devkit-patterns)
- React/Vue search UI components (use mapbox-web-integration-patterns)
- Style customization for search results (use mapbox-style-patterns)
...
- mapbox-search-patterns - Which search tool to use, parameter optimization
- mapbox-mcp-devkit-patterns - MCP bulk operations
- mapbox-web-integration-patterns - React/Vue integration patterns
- mapbox-standards - Studio workflows and data flow

---

## Parallel Execution Strategy

**Search integration has distinct phases with parallelization opportunities:**

```
Phase 1: Discovery (PARALLEL - Run concurrently)
┌──────────────────────────────────────────────┐
│ Task A: Ask discovery questions              │
│ Task B: Read project context (package.json)  │
│ Task C: Check existing search implementation │
└──────────────────┬───────────────────────────┘
                   ↓
Phase 2: Product Selection (SEQUENTIAL)
• Analyze answers → Choose Search Box vs Geocoding API
                   ↓
Phase 3: Setup (PARALLEL - Run concurrently)
┌──────────────────────────────────────────────┐
│ Task D: Install dependencies (npm install)   │
│ Task E: Read framework integration patterns  │
│ Task F: Review token configuration           │
└──────────────────┬───────────────────────────┘
                   ↓
Phase 4: Implementation (SEQUENTIAL)
• Build search component → Add to app → Test
                   ↓
Phase 5: Testing + Docs (PARALLEL - Run concurrently)
┌──────────────────────────────────────────────┐
│ Task G: Test search functionality            │
│ Task H: Write integration documentation      │
└───────────────────────────────────────────────┘
```

**Time savings:** 30-40% via parallelization of discovery, setup, and testing phases
...
<html>
  <head>
    ...
    <link
      href="https://api.mapbox.com/search-js/v1.0.0-beta.18/web.css"
      rel="stylesheet"
    />
...
fetch(searchUrl, { signal: controller.signal }).finally(() =>
