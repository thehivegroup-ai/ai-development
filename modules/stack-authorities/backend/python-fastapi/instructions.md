...
# 32: API - Python + FastAPI

**Authority Type:** Backend Framework Stack  
...
Enforce FastAPI patterns, Pydantic validation, async/await best practices, and Python type hints for API development.
...
**Rationale:** Type safety, better IDE support, runtime validation with Pydantic.
...
Use FastAPI's dependency injection system for database sessions, auth, etc.
...
print(settings.DATABASE_URL)  # From environment
```
...
class UserInDB(UserResponse):
    """Properties stored in database (includes hashed password)"""
    hashed_password: str
```
...
- Uvicorn (ASGI server)
- PostgreSQL (or other async-supported database)
- python-jose (JWT)
